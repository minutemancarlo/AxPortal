$(document).ready(function() {
  ...
  function getRequestStatus() {
    $.ajax({
      ...
      success: function(response) {
        ...
        $("#councilorCount").html(data.Chancellor);
        ...
        switch ($('meta[name="rl"]').attr('content')) {
          case "0":
          var counter = data.Pending;
            $("#approvalCount").html(counter);
            break;
          case "1":
          var counter = data.Pending + data.Chancellor ;
          console.log(data.Councilor);
            $("#approvalCount").html(counter);
            break;
          case "3":
          var counter = data.Auxiliary;
            $("#approvalCount").html(counter);
            break;
        }
  ...
  var table = $('#requestsTable').DataTable({
    ...
    ajax: {
      ...
      type: "POST",
      data: {
        action: 'select-all-requests-user'
      },
      dataType: 'json',
      dataSrc: '',
      cache: false
    ...
    createdRow: function(row, data, dataIndex) {
      ...
      switch ($('meta[name="rl"]').attr('content')) {
        case "0":
          ...
          break;
        case "1":
          if (statusHtml == "For Approval" || statusHtml == "Pending") {
            count++;
  ...
  $('#requestsTable tbody, #ApproverequestsTable tbody').on('click', '.request_id', function() {
    ...
    $.ajax({
      ...
      type: 'POST',
      data: {
        action: "select-by-request",
        rid: $("#requestModal_id").html()
      }, // corrected selector
      dataType: 'json',
      success: function(response) {
