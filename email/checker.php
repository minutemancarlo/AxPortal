...
if (file_exists($lockFilePath)) {
    ...
    file_put_contents('C:\wamp64\www\axportal\email\service_logs\log.txt', date('Y-m-d H:i:s') . " Script skipped (already running)\n", FILE_APPEND);
    exit;
}
...
if (!$lockFile) {
    echo "Error creating lock file.";
    file_put_contents('C:\wamp64\www\axportal\email\service_logs\log.txt', date('Y-m-d H:i:s') . " Error creating lock file\n", FILE_APPEND);
    exit;
}
...
// The script is now locked and can proceed
...
// Remove the lock file, indicating that the script has finished
unlink($lockFilePath);

file_put_contents('C:\wamp64\www\axportal\email\service_logs\log.txt', date('Y-m-d H:i:s') . " Script executed\n", FILE_APPEND);

function processRecord($row, $baseURL) {
    // Your existing logic for processing each record and sending emails
    $requestNo = $row['rid'];
    if ($row['level'] == 0) {
    ...
    // Make sure to adjust any absolute paths used in the original script
}
?>
